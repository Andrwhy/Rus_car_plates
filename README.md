...
Прототип ориентирован на автоматизацию регистрации транспорта на КПП, парковках и закрытых территориях: сотрудник отправляет фотографию автомобиля боту, а система находит государственный номер, распознаёт его и возвращает нормализованную строку.
...
## Задача и практическая ценность

**Проблема.** Ручная проверка и регистрация автомобилей на контрольно-пропускном пункте занимает время, зависит от внимательности сотрудника и приводит к ошибкам при вводе номера.

**Решение.** Сотрудник использует привычный интерфейс Telegram: отправляет фото автомобиля, после чего двухэтапный пайплайн локализует номер с помощью YOLO11, распознаёт символы специализированной CRNN-моделью с BiLSTM/CTC и нормализует результат под российский формат. Если обученные OCR-веса недоступны, используется резервный EasyOCR.

**Бизнес-эффект.** Подход сокращает объём ручного ввода, снижает вероятность опечаток и ускоряет идентификацию автомобиля. Архитектуру можно подключить к списку разрешённого транспорта, СКУД или цифровому журналу въезда и выезда — для автоматической проверки и фиксации событий.

В текущем эксперименте детектор достиг **97,73% mAP@50**, **95,09% Precision** и **94,67% Recall**. На независимом наборе вырезанных номеров OCR распознаёт полный номер с точностью **85,03%**, отдельные символы — с точностью **97,59%**.
