...
`.env` исключён из Git. Если токен бота когда-либо попал в консоль, логи или публичный коммит, немедленно перевыпустите его через BotFather и обновите локальный `.env`.
